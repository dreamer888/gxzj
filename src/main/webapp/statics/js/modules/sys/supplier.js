...
var vm = new Vue({
    ...
    methods: {
        ...
        update: function (event) {
            ...
            vm.showList = false;
            vm.title = "修改";
            vm.supplier.headImgUrl = vm.headImgUrl;
            vm.getInfo(id)
        },
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                        type: "POST",
                        url: baseURL + "sys/supplier/delete",
                        contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "sys/supplier/info/" + id, function (result) {
                vm.supplier = result.supplier;
                var url = vm.supplier.headImgUrl;
                vm.headImgUrl = url;
                $(".logo img").attr("src", url);
                $(".logo img").attr("layer-src", url);
                vm.supplier.password = null;
            });
        },
        test:function () {
            var value = $('#testSelect option:selected') .val();
            vm.q.temp = value;
        },
        ...
        validator: function () {
            ...
            if (vm.supplier.phone.length>15) {
                alert("电话号码过长");
                return true;
            }
            if (!phoneReg.test(vm.supplier.phone)) {
                alert('请输入有效的手机号码！');
                return true;
            }
            if(!passwordReg.test(vm.supplier.password)){
                alert("密码只能是数字或者字母");
                return true;
            }
            if (vm.supplier.status == null) {
                alert("用户状态不能为空");
                return true;
            }
